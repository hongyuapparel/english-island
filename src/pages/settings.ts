  ...
  el.innerHTML = `
    ...
    <form class="card form-grid" id="settings-form">
      ...
      <div class="provider-section full">
        <h3>🔊 朗读语音</h3>
        <p class="hint">免费语音同样无需 Key、温暖自然；系统语音离线可用但机械。</p>
        <div class="field">
          <label for="ttsVoice">语音类型</label>
          <select id="ttsVoice">
            <option value="free" ${settings.ttsVoice === 'free' ? 'selected' : ''}>免费语音（无需 Key，推荐）</option>
            <option value="natural" ${settings.ttsVoice === 'natural' ? 'selected' : ''}>Gemini 语音（需 Key）</option>
            <option value="system" ${settings.ttsVoice === 'system' ? 'selected' : ''}>系统语音（离线）</option>
          </select>
        </div>
